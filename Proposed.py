...
#  Enhanced Geyser inspired Algorithm (EGEA)  Starting Line No. 48
def Proposed(population, fobj, VRmin, VRmax, max_iterations):
    ...
    for iteration in range(max_iterations):
        ...
        for i in range(n_pop):
            ...
            currentfit = new_cost_1[i]
            worstfit = np.max(new_cost_1)
            meanfit = np.mean(new_cost_1)
            r = currentfit / (worstfit + (meanfit / 100) + currentfit)
            pressure_value = r  # Replace with actual pressure calculation
